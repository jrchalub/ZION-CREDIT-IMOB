  ...
  return (
    <div className="space-y-8">
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-600">
            Indicadores internos de pré-análise — sem aprovação bancária automática.
          </p>
        </div>
        <Link
          href="/processes"
      ...
      <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <Metric label="Pré-aprováveis" value={metrics.totals.preAprovaveis} />
        <Metric label="Reprováveis (interno)" value={metrics.totals.reprovaveis} />
        <Metric label="Enviados ao banco" value={metrics.totals.enviadosAoBanco} />
        <Metric label="Aprovados (IF)" value={metrics.totals.aprovados} />
      ...
      <section className="grid gap-6 lg:grid-cols-2">
        ...
        <div className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
          <h2 className="font-serif text-xl">Tempo médio</h2>
          <p className="mt-4 font-serif text-4xl text-slate-900">
            {metrics.avgAnalysisHours.toFixed(1)}h
          </p>
          <p className="mt-2 text-sm text-slate-600">
            Média entre abertura e última movimentação dos processos do tenant.
          </p>
        </div>
      </section>
    </div>
